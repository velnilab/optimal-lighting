...
#############################################################################################
#############################################################################################
# lighting strategies, sunlight plot


'''
#first fig
fig, axs = plt.subplots(2) #number of figures in subplot
axs[0].plot(t,S1_PPFD,label = 'Predicted Sunlight',color='red', marker='o') # marker='o' puts circle dots on values  
axs[0].plot(t,S_Test_PPFD,label = 'Real Sunlight',color='black', marker='o', linestyle='dashed')
#axs[0].set(xlabel='time(hour)')
#axs[0].set_ylim(0,700)
axs[0].set_xlim(4.5,20.5)
axs[0].set(ylabel='PPFD')
axs[0].set_title('Sunlight Prediction')
axs[0].legend(loc="upper right",frameon=False,fontsize='medium') 
axs[0].grid()
fig.tight_layout(pad=2.0) #adds some space between two figures
#second Fig
axs[1].plot(t,X_Light_PPFD,label = 'Prediction_based Method',color='red', marker='o')
axs[1].plot(t,X_Light1_PPFD,label = 'Heuristic Method',color='green', marker='*')
axs[1].plot(t,x_complete_information_PPFD,label = 'Baseline',color='blue', marker='*')
axs[1].plot(t,S_Test_PPFD,label = 'Sunlight data',color='black', marker='o', linestyle='dashed')
axs[1].set(xlabel='time(hour)')
#axs[1].set_ylim(0,700)
axs[1].set_xlim(4.5,20.5)
axs[1].set(ylabel='PPFD')
axs[1].set_title('Lighting Strategy')
axs[1].legend(loc="upper right",frameon=False,fontsize='medium') 
axs[1].grid()
####increasing the size of figure(saving high quality)
fig = plt.gcf()
fig.set_size_inches(10.5, 10.5)
filename='lighting_strategy'+str(day)
fig.savefig('/home/pi/Shirin/'+filename, dpi=400)

# Driver Voltage plot
fig.clear()
X_PWM_p = np.zeros((len(X_Light_PPFD),1))
for ww in range(0,len(X_Light_PPFD)):
    X_PWM_p[ww,0] = (X_Light_PPFD[ww,0]/400)*100
    
X_PWM_h = np.zeros((len(X_Light1_PPFD),1))
for www in range(0,len(X_Light1_PPFD)):
    X_PWM_h[www,0] = (X_Light1_PPFD[www,0]/400)*100
    
plt.plot(t,X_PWM_p,label = 'Prediction_based Method',color='blue', marker='o')
plt.plot(t,X_PWM_h,label = 'Heuristic Method',color='green', marker='*')
plt.xlim(4.5,20.5)
plt.ylim(0,110)
plt.xlabel('time(hour)')
plt.ylabel("Normalized Voltage(%)")
plt.title('driver voltage for two methods')
plt.legend(loc="upper right",frameon=False,fontsize='medium')
plt.grid()
plt.gcf().set_size_inches(10.5,5)
filename2='duty_cycle'+str(day)
plt.gcf().savefig('/home/pi/Shirin/'+filename2, dpi=400)
plt.clf()
'''